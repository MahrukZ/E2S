...
export class ConsumptionController {
  ...
  async bulkCreateConsumptions(req: Request, res: Response): Promise<any> {
    return this.consumptionService
      .bulkCreateConsumptions(req.body)
      .then((data) => {
        res.status(201).json({
          message: "Created",
          status: 201,
      ...
      .catch((err) => {
        res.status(500).json({
          message:
            err.message || "server error: failed to bulk create consumptions.",
          status: 500,
        });
      });
  }
  ...
  async createConsumption(req: Request, res: Response): Promise<any> {
    return this.consumptionService
      ...
      .catch((err) => {
        res.status(500).json({
          message:
            err.message || "server error: failed to create a consumption.",
          status: 500,
        });
      });
  }
