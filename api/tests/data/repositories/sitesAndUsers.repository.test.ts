...
import {
  SitesAndUsers,
  ISitesAndUser,
} from "../../../data/models/sitesAndUsers.model";
...
describe("SitesAndUsersRepository", () => {
  ...
  describe("SitesAndUsersRepository.getAllSitesAndUsers", () => {
    ...
    it("should not fetch insights when there is no data in the database", async () => {
      ...
      sitesAndUsersRepository.getAllSitesAndUsers = jest
        .fn()
        .mockRejectedValue(mErrorMessage);

      // Then
      expect(sitesAndUsersRepository.getAllSitesAndUsers).rejects.toMatchObject(
        mErrorMessage
      );
      expect(sitesAndUsersRepository.getAllSitesAndUsers).toHaveBeenCalledTimes(
        1
      );
      expect(
        sitesAndUsersRepository.getAllSitesAndUsers
  ...
  describe("SitesAndUsersRepository.findSitesAndUsersByUserId", () => {
    it("should find sites and users when Id is provided", async () => {
      ...
      // When
      const result = await sitesAndUsersRepository.findSitesAndUsersByUserId(
        userId
      );
    ...
    it("should not find sites and users when there is no Id provided", async () => {
      ...
      sitesAndUsersRepository.findSitesAndUsersByUserId = jest
        .fn()
        .mockRejectedValue(mErrorMessage);

      // Then
      expect(
        sitesAndUsersRepository.findSitesAndUsersByUserId
