...
import {
  UserManagement,
  IUserManagement,
} from "../../../data/models/userManagement.model";
...
describe("UserManagementRepository", () => {
  ...
  describe("UserManagementRepository.getAllUserManagements", () => {
    ...
    it("should not fetch user management data when there is no data in the database", async () => {
      ...
      userManagementRepository.getAllUserManagements = jest
        .fn()
        .mockRejectedValue(mErrorMessage);

      // Then
      expect(
        userManagementRepository.getAllUserManagements
  ...
  describe("UserManagementRepository.findUserManagementByUserId", () => {
    it("should find user management when Id is provided", async () => {
      ...
      // When
      const result = await userManagementRepository.findUserManagementByUserId(
        userId
      );
    ...
    it("should not find user managements when there is no Id provided", async () => {
      ...
      userManagementRepository.findUserManagementByUserId = jest
        .fn()
        .mockRejectedValue(mErrorMessage);

      // Then
      expect(
        userManagementRepository.findUserManagementByUserId
      ).rejects.toMatchObject(mErrorMessage);
      expect(
        userManagementRepository.findUserManagementByUserId
      ).toHaveBeenCalledTimes(1);
